...
sci1freq<- function(mvars,x,data,dec,nonnormal=NULL) {
  ...
  for (i in (1:nc)) {
    ...
    d1<-cbind(rownames(t1),tb1,rep(" ",times=length(rownames(t1))))
    d2<-c(mvars[i],rep(" ",times=n.x),pp1)
    d0<-rbind(d2,d1)
    d<-rbind(d,d0)
  }
  tmp.cname1<-paste(x,".",levels(factor(xvt)),sep="")
  colnames(d)<-c("Characteristic",tmp.cname1,"p value")
  d
}
